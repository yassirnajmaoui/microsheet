...
# TODO:
# Fix isse with the modification of the cell vs modification of the formula
# - Remove automatic runs (make it so that the user explicitly asks to run a cell or run the sheet)
# - Make it so that sheets that were modified but not run are of a specific color
#	 Have a state machine for each cell?
# - Add small console and use the "code" module (builtin)
#	- Make it so that the explicit runs are actually just commands run in the console (So every interaction with the kernel goes through the small console)
